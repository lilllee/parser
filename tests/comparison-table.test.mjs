...
console.log("\n[2c] hasUnitJammedCells: 단위가 값과 한 셀에 뭉친 표 감지(정상 표는 미감지)");
{
  // kordoc 이 단위/실적/목표 열을 못 나눠 한 셀에 '명 1,180 1,109' 뭉친 망가진 표
  const jammed = { rows: 2, cols: 3, cells: [
    [{ text: "대체인력 지원 확대" }, { text: "명 1,180 1,109" }, { text: "" }],
    [{ text: "공공보육 이용률 확대" }, { text: "% 46 50" }, { text: "" }],
  ] };
  // 정상 표: 단위와 값이 별도 셀
  const okTable = { rows: 2, cols: 4, cells: [
    [{ text: "대체인력 지원 확대" }, { text: "명" }, { text: "1,180" }, { text: "1,109" }],
    [{ text: "공공보육 이용률 확대" }, { text: "%" }, { text: "46" }, { text: "50" }],
  ] };
  ok(hasUnitJammedCells(jammed) === true, "'명 1,180 1,109' 단위뭉침 → 감지");
  ok(hasUnitJammedCells(okTable) === false, "단위/값 분리된 정상 표 → 미감지");
}

console.log("\n[2d] isMangledTable: 좋은 HTML 병합 표는 면제, 값 뭉친 표는 구조 무관하게 감지");
{
  // 좋은 kordoc HTML 병합 표(colspan/rowspan, 값 뭉침 없음) — %주석·병합 빈셀이 있어도 면제
  const goodMerged = {
    rows: 2, cols: 4,
    cells: [
      [{ text: "구분", colSpan: 1, rowSpan: 2 }, { text: "2025년", colSpan: 2 }, { text: "", colSpan: 1 }, { text: "비고" }],
      [{ text: "" }, { text: "목표" }, { text: "실적" }, { text: "균특 50% 도비 25% 시군비 25%" }],
    ],
  };
  // 병합 구조여도 한 셀에 값이 뭉친 망가진 표(인구동향 p6 사례)
  const mergedButJammed = {
    rows: 2, cols: 3,
    cells: [
      [{ text: "구분", colSpan: 3 }],
      [{ text: "전국" }, { text: "238,317 254,457 21,112" }, { text: "" }],
    ],
  };
  ok(hasJammedNumberCell(mergedButJammed) === true, "'238,317 254,457 21,112' 값뭉침 → 감지");
  ok(hasJammedNumberCell(goodMerged) === false, "좋은 표(셀당 1값) → 값뭉침 미감지");
  ok(isMangledTable(goodMerged) === false, "좋은 HTML 병합 표 → 망가짐 아님(면제)");
  ok(isMangledTable(mergedButJammed) === true, "병합이어도 값 뭉치면 → 망가짐 감지");
}

console.log("\n[2e] 예산표 보조율 문구는 percentCram 오탐 없이 두고, 붙은 금액은 감지한다");
{
  const budgetRateTable = {
    rows: 4, cols: 3,
    cells: [
      [{ text: "구분" }, { text: "지원내용" }, { text: "지원금액\n(도비 50~100%, 시군비 50%~0%)" }],
      [{ text: "신축" }, { text: "건축비 및 설계·용역비" }, { text: "총사업비의 10%\n최대 3억원(도비100%)" }],
      [{ text: "기존 전환" }, { text: "리모델링비" }, { text: "최대 1.2억원\n(도비50%, 시군비 50%)" }],
      [{ text: "신규개원 안착지원" }, { text: "운영비 등" }, { text: "최대 0.2억원\n(도비50%, 시군비 50%)" }],
    ],
  };
  const percentArtifact = {
    rows: 1, cols: 1,
    cells: [[{ text: "48.9% 23.7% 41.6%" }]],
  };
  const gluedMoney = {
    rows: 2, cols: 3,
    cells: [
      [{ text: "리모델링" }, { text: "6,167,500990,000" }, { text: "4,805,625760,500" }],
      [{ text: "신규개원" }, { text: "1,580,00050,000" }, { text: "790,00025,000" }],
    ],
  };
  ok(isPercentCramTable(budgetRateTable) === false, "보조율 설명 표 → percentCram 미감지");
  ok(isMangledTable(budgetRateTable) === false, "정상 보조율 표 → reflow 대상 아님");
  ok(isPercentCramTable(percentArtifact) === true, "퍼센트값만 뭉친 차트 잔해 → 감지");
  ok(hasGluedCommaNumberCell(gluedMoney) === true, "쉼표 금액 두 개가 붙은 셀 → 감지");
  ok(isMangledTable(gluedMoney) === true, "붙은 금액 표 → reflow 대상");
}
